struct StepperBase {
	...
	double hnext;
	double EPS;
	int n,neqn,real_nvar;
	VecDoub yout,yerr;
	StepperBase(VecDoub &yy, VecDoub &dydxx, double &xx, const double atoll,const int real_nvarr,
		const double rtoll, bool dens) : x(xx),y(yy),dydx(dydxx),atol(atoll),
		rtol(rtoll),dense(dens),n(y.size()),neqn(n),real_nvar(real_nvarr),yout(n),yerr(n) {}
};
...
struct StepperDopr853 : StepperBase, Dopr853_constants {
	...
	StepperDopr853(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
		const Doub atoll,const Int real_nvarr, const Doub rtoll, bool dens);
...
StepperDopr853<D>::StepperDopr853(VecDoub_IO &yy,VecDoub_IO &dydxx,Doub &xx,
	const Doub atoll,const Int real_nvarr,const Doub rtoll,bool dens) :
	StepperBase(yy,dydxx,xx,atoll,real_nvarr,rtoll,dens),yerr2(n),k2(n),k3(n),k4(n),
...
Doub StepperDopr853<D>::error(const Doub h) {
	...
	if (deno <= 0.0)
		deno=1.0;
	return abs(h)*err*sqrt(1.0/(real_nvar*deno));
}
template <class D>
...
bool StepperDopr853<D>::Controller::success(const Doub err, Doub &h) {
	static const Doub beta=0.04,alpha=1.0/8.0-beta*0.2,safe=0.9,minscale=0.333,
		maxscale=6.0;
	Doub scale;
	if (err <= 1.0) {
		if (err == 0.0)
